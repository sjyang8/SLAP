...
------------------------------------------------------------------------------------------
The package includes the MATLAB code of the SLAP algorithm in paper "Superpixelwise Low-rank Approximation based Partial Label Learning for Hyperspectral Image Classification" [1].
...
For SLAP,  the "call_SLAP.m" will automatically create a new folder for saving the detailed results,  i.e. "Indian_pinesSP64SVM_results/lambda1gama20/ratio0.07/r1/". In such folder, 
for example, the result file "Indian_pinesSP64lambda1gama20alpha0.96per_C0.07.mat" saves the results for 10 runs.  In the following, we also show the detailed description for the 
variables that stored in the result file.
Variable description:
accuracy_SVM1 	-------- 	OA on original feature (10 runs)
accuracy_SVM2  	-------- 	OA on denoising feature by SLAP (10 runs)
Kappa_SVM1     	--------	Kappa on original feature (10 runs)
Kappa_SVM2    	--------	Kappa on denoising feature by SLAP (10 runs)
TPR_SVM1         	--------	Accuracy of each class on original feature (10 runs)
TPR_SVM2  	--------    Accuracy of each class on denoising feature by SLAP (10 runs)
Predict_SVM1	--------	The prediction on original feature (10 runs)
Predict_SVM2	--------	The prediction on denoising feature by SLAP (10 runs)
...
res  --------- the average result summary for 10 runs
	 res. ave_OA_SVM1 ------- the average OA for 10 runs on original feature
	 res. ave_OA_SVM2 ------- the average OA for 10 runs by SLAP
	 res. ave_AA_SVM1 ------- the average AA for 10 runs on original feature
	 res. ave_AA_SVM2 ------- the average AA for 10 runs by SLAP
	 res. ave_Kappa_SVM1 ------- the average Kappa for 10 runs on original feature
	 res. ave_Kappa_SVM2 ------- the average Kappa for 10 runs by SLAP
	 res. ave_TPR_SVM1 ------- the average Accuracy of each class for 10 runs on original feature 
	 res. ave_TPR_SVM2 ------- the average Accuracy of each class for 10 runs by SLAP 
	 res. OA_SVM1_std ------- the average std of OA for 10 runs on original feature
	 res. OA_SVM2_std ------- the average std of OA for 10 runs by SLAP
	 res. AA_SVM1_std ------- the average std of AA for 10 runs on original feature
	 res. AA_SVM2_std ------- the average std of AA for 10 runs by SLAP
	 res. Kappa_SVM1_std ------- the average std of Kappa for 10 runs on original feature
	 res. Kappa_SVM2_std ------- the average std of Kappa for 10 runs by SLAP
	 res. TPR_SVM1_std ------ the average std of Accuracy of each class for 10 runs on original feature 
